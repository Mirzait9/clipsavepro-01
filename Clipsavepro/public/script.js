function initClipSavePro() {
    ...
    // Client-Side Direct Extraction Action
    if (downloadBtn && urlInput) {
        const handleDownload = async function(e) {
            ...
            downloadBtn.innerText = 'Extracting Stream Link...';

            try {
                // Direct call from user browser bypassing Render Server Blocking completely!
                ...
                if (data && data.url) {
                    resultDiv.innerHTML = '<p style="margin-bottom: 12px; font-weight: bold; color: #28a745; font-size: 1.1rem;">Video Ready!</p><a href="' + data.url + '" target="_blank" rel="noopener noreferrer" style="display: inline-block; background: #28a745; color: white; text-decoration: none; padding: 12px 24px; border-radius: 10px; font-weight: bold; box-shadow: 0 4px 15px rgba(40,167,69,0.2);">Click Here to Save Video</a>';
                    resultDiv.style.display = 'block';
                } else if (data && data.text) {
                    errorDiv.innerText = data.text;
                    errorDiv.style.display = 'block';
                } else {
                    errorDiv.innerText = 'Could not extract this video. Make sure it is a public YouTube video.';
                    errorDiv.style.display = 'block';
                }
            } catch (error) {
                console.error("Direct fetch failed:", error);
                loader.style.display = 'none';
                errorDiv.innerText = 'Connection stable error or invalid response. Please try again.';
                errorDiv.style.display = 'block';
            } finally {
                loader.style.display = 'none';
